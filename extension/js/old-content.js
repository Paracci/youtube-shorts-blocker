/**
 * ARCHIVE FILE - v1.11.0
 * 
 * This file contains the complete, monolithic version of the content script logic as of v1.10.5.
 * For v1.11.0, the extension has been fully modularized into smaller, focused files:
 * - ad-blocker.js
 * - channel-blocker.js 
 * - downloader.js
 * - premium-detector.js
 ...
 * - etc.
 * 
 * This file is kept in the repository for archival purposes only and is NOT used by the current manifest.
 */

// ─────────────────────────────────────────────────────────────────────────────
// YouTube Shorts Channel Blocker + Video Downloader – content.js (LEGACY ARCHIVE)
// ─────────────────────────────────────────────────────────────────────────────
...
let _adNavPending = false;      // debounce: prevent double-skip when multiple renderers fire at once
...
function skipCurrentAd() {
    ...
    // 2. Unskippable ad playing — mute and fast-forward to end
    if (player && adActive) {
    ...
    // 3. Ad just ended — restore audio and resume the main video cleanly
    //    This eliminates the black-screen / silent hang after the ad disappears.
    if (_adMutedByUs && player) {
        player.muted = false;
        _adMutedByUs = false;
...
// Navigate to the next Short without touching the ad renderer node.
// We try three methods in order:
//   1. Click the dedicated "next" chevron button YouTube renders in the Shorts player
//   2. Dispatch an ArrowDown keydown on the document (YouTube's own keyboard handler)
//   3. Fall back to ArrowDown on the active video element
function navigateToNextShort() {
    // Method 1 – Shorts navigation button (most reliable)
    ...
    // Method 2 – ArrowDown on document (YouTube handles this natively for Shorts)
...
function checkForAdAndSkip(renderer) {
    ...
    // Mute immediately so the user never hears the ad
    const video = renderer.querySelector('video');
    if (video && !video.muted) { video.muted = true; _adMutedByUs = true; }

    // Navigate away from the ad straight to the next Short.
    // Guard with _adNavPending so that if multiple renderers in the DOM are
    // flagged as ads in the same MutationObserver / IntersectionObserver batch,
    // we only fire one navigation — otherwise two ArrowDown events are dispatched
    // and a normal Short gets skipped along with the ad.
    if (!_adNavPending) {
        _adNavPending = true;
        setTimeout(() => {
            _adMutedByUs = false;
            navigateToNextShort();
            // Increment the ads-blocked counter shown in the popup
            safeStorageGet('adsBlockedCount', (res) => {
                safeStorageSet({ adsBlockedCount: (res.adsBlockedCount || 0) + 1 });
            });
            // Reset after a safe window so future real ads can be skipped again
            setTimeout(() => { _adNavPending = false; }, 1500);
        }, 80);
    }

    // !! Do NOT set renderer.style.display = 'none' !!
    // YouTube recycles ytd-reel-video-renderer nodes. Hiding the node causes
    // subsequent normal Shorts to render inside a hidden container — video
    // freezes while audio continues to play.
    return true;
}
